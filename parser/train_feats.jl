# To extend the train.jl file to morphological features
...
function main(o)
    ...
    wmodel = makewmodel(d)
    corpora = []
    lfunc = loadcorpus_v62
    #lfunc = (VERSION >= v"0.6.0" ? loadcorpus_v6 : loadcorpus)
    for f in o[:datafiles]; @msg f # TODO: you need to check number of datafiles
        if length(o[:datafiles]) > 1
            c, fdict = lfunc(f,vocab)
        end
        c = lfunc(f,vocab)
        push!(corpora,c)
    end
    ...
    # add feature selector for the featmodel
    featmodel = 
